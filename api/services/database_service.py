...
class DatabaseService:
    ...
    def create_user(self, platform_id: str, platform_name: str, name: str, phone: str) -> Dict[str, Any]:
        """Create or update a user."""
        try:
            inserter = self.get_inserter(platform_id)
            inserter.upsert_client(platform_name=platform_name, name=name, phone=phone)
            
            logger.info(f"User data inserted successfully for platform_id: {platform_id}")
            return {
    ...
    def create_transaction(self, platform_id: str, **transaction_data) -> Dict[str, Any]:
        """Create a new transaction."""
        try:
            inserter = self.get_inserter(platform_id)
            transaction_result = inserter.insert_transaction(**transaction_data)
            
            logger.info(f"Transaction created successfully for platform_id: {platform_id}")
            return {
                "platform_id": platform_id,
    ...
    def create_limit(self, platform_id: str, category_id: str, limit_value: float) -> Dict[str, Any]:
        """Create a new limit."""
        try:
            inserter = self.get_inserter(platform_id)
    ...
    def update_transaction(self, platform_id: str, transaction_id: int, **update_data) -> Dict[str, Any]:
        """Update a transaction."""
        try:
            inserter = self.get_inserter(platform_id)
            inserter.update_transaction(transaction_id=transaction_id, data=update_data)
            
            logger.info(f"Transaction updated successfully for platform_id: {platform_id}")
            return {
                "platform_id": platform_id,
    ...
    def delete_transaction(self, platform_id: str, **delete_data) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Transaction(s) deleted successfully for platform_id: {platform_id}")
            return {
                "platform_id": platform_id,
                **delete_data
            }
    ...
    def grant_subscription(self, platform_id: str, subscription_months: int) -> Dict[str, Any]:
        """Grant a subscription to a user."""
        try:
            inserter = self.get_inserter(platform_id)
            inserter.grant_subscription(subscription_months=subscription_months)
            
            logger.info(f"Subscription granted successfully for platform_id: {platform_id}")
            return {
                "platform_id": platform_id,
    ...
    def revoke_subscription(self, platform_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Subscription revoked successfully for platform_id: {platform_id}")
            return {
                "platform_id": platform_id
            }
        except ClientNotExistsError as e:
